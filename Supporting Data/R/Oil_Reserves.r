#' Calculate the oil reserves using several different exponential and harmonic declines.
#' 
#' \code{Oil_Reserves}
#'
...
Oil_Reserves <- function(x, ReleaseDate=FALSE){
  ...
  if(ReleaseDate) {
    ...
    if(length(q)<3) {
      ...
      # Check if the well has more than 9 producing months before doing a decline
      if(length(q)>9) {
        Cum_Prod <- max(getDataPart(Q))/1000
